 ...
 */
class AllPay_CVS_Block_Redirect extends Mage_Core_Block_Template {
    ...
    protected function AutoSubmit() {
        ...
        if ($oOrder) {
            try {
                ...
                $oPayment->Send['ReturnURL'] = Mage::getUrl('cvs/processing/response');
                $oPayment->Send['ClientBackURL'] = Mage::getUrl('');
                $oPayment->Send['OrderResultURL'] = Mage::getUrl('cvs/processing/result');
                ...
                array_push($oPayment->Send['Items'], array('Name' => Mage::helper('cvs')->__('Commodity Group'), 'Price' => (int) $oOrder->getGrandTotal(), 'Currency' => Mage::app()->getLocale()->currency(Mage::app()->getStore()->getCurrentCurrencyCode())->getSymbol(), 'Quantity' => 1, 'URL' => ''));

                $oPayment->SendExtend["Desc_1"] = '';
                $oPayment->SendExtend["Desc_2"] = '';
                $oPayment->SendExtend["Desc_3"] = '';
                $oPayment->SendExtend["Desc_4"] = '';
                $oPayment->SendExtend["PaymentInfoURL"] =  Mage::getUrl('cvs/processing/response');
