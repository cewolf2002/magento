 ...
 * @author shawn.chang
 */
class AllPay_CVS_ProcessingController extends Mage_Core_Controller_Front_Action {
    ...
    public function redirectAction() {
        try {
            ...
            if (!$oOrder->getId()) {
                Mage::throwException(Mage::helper('cvs')->__('Order No not found'));
            }
            if ($oOrder->getState() != Mage_Sales_Model_Order::STATE_NEW) {
                $oOrder->setState(
                        Mage_Sales_Model_Order::STATE_NEW, Mage_Sales_Model_Order::STATE_NEW, Mage::helper('cvs')->__('Redirect to pay page')
                )->save();
    ...
    protected function _processSale($request) {
        ...
        try {
            ...
            if ($request['RtnCode'] == '10100073' && $oOrder->getState() == Mage_Sales_Model_Order::STATE_NEW) {
                $szPaymentType = $request['PaymentType'];
                ...
                $szComment = sprintf(Mage::helper('cvs')->__('CVS Payment Information'), $szPaymentType, $szTradeDate, $szBankCode, $szExpireDate, $szBarcode1, $szBarcode2, $szBarcode3);
